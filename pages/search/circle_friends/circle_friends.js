...
Page({
  ...
  GetNeighborList: function() {
    ...
    base.postAjaxEntity({
      ...
      for (var index in data.rows) {
        ...
        data.rows[index].Neighbor.PhotoList = PhotoList;
  ...
  getLSImg: function(rows, imglist, sum, mapList) {
    ...
    var downLoadImg = imglist[rows];
    if (downLoadImg == null || downLoadImg == "") {
      rows++;
      if (rows < sum) {
    ...
    if (downLoadImg != null && downLoadImg.indexOf('https:') > -1) {
      //var imgNewList = downLoadImg.replace("http:/", '').replace("https:/", '');
      list.push(downLoadImg);
      rows++;
      if (rows < sum) {
        that.getLSImg(rows, imglist, sum, mapList);
      } else {
